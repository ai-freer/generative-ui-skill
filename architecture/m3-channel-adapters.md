...
| 能力 | Aight (iOS) | Web (Playground) | 飞书 Bot | Telegram Bot | QQ Bot |
|------|:-----------:|:----------------:|:--------:|:------------:|:------:|
| 嵌入任意 HTML/JS | ✅ WKWebView | ✅ iframe | ❌ | ❌ | ❌ |
| 流式 DOM 预览 | ✅ | ✅ | ❌ | ❌ | ❌ |
| 富文本卡片 | — | — | ✅ 交互卡片 | ❌ | ✅ Ark 消息 |
| 按钮回调 | ✅ native | ✅ JS | ✅ 卡片按钮 | ✅ inline keyboard | ✅ 按钮交互 |
...
show-widget 围栏
  ...
  → 发送链接（飞书 H5 / Telegram Mini App / QQ 小程序）
...
| Widget 类型 | Aight / Web | 飞书 | Telegram | QQ |
|------------|:-----------:|:----:|:--------:|:--:|
| SVG 流程图/架构图 | A | B | B | B |
...
show-widget 围栏
  ...
  │
  ├─ 结构化 widget（指标卡片/表格）
  │   → 策略 B：截图为主（QQ Ark 模板灵活度不如飞书卡片）
  │   → 附加 drill-down 按钮
  │
  └─ 交互型 widget（计算器/可操作图表）
      → 策略 D：widget_code 存储 → QQ 小程序链接
      → 消息内嵌"打开交互版"按钮
```
